...
export type MoneyHealthFactors = {
  savingsRate: number;
  debtToIncome: number;
...
export type MoneyHealthResult = {
  score: number;
  label: string;
  color: "red" | "yellow" | "green";
  factors: MoneyHealthFactors;
...
function scoreFromSavingsRate(pct: number): number {
...
function scoreFromDTI(pct: number): number {
...
function scoreFromEmergency(months: number): number {
...
function scoreFromInvestmentRatio(pct: number): number {
...
function scoreFromStability(score0to100: number): number {
  return clamp(score0to100, 20, 100);
}

function scoreFromCredit(score0to100: number): number {
...
export function computeMoneyHealth(input: {
  ...
  const factors: MoneyHealthFactors = {
...
export function projectSipGrowth(params: {
  ...
  for (let y = 1; y <= params.years; y++) {
    const months = y * 12;
    let fv = 0;
    if (r === 0) fv = pmt * months;
    else fv = pmt * ((Math.pow(1 + r, months) - 1) / r);
...
export function fireMonthlySip(params: {
  ...
  annualReturnPct: number;
}): { monthlySip: number; monthsToGoal: number; yearsToRetirement: number } {
  ...
  let monthly = 0;
  if (r === 0) monthly = gap / months;
  else monthly = (gap * r) / (Math.pow(1 + r, months) - 1);
  return {
...
export function computeTaxIndia(params: {
  ...
  const oldTaxable = Math.max(
    params.grossSalary - std - Math.min(params.deductions80C, 150000) - params.deductionsOther,
    0
  );
...
export const LIFE_EVENT_PRESETS: Record<
  string,
  { label: string; emoji: string; allocation: { invest: number; save: number; spend: number; insurance: number } }
> = {
  bonus: {
    label: "Bonus",
    emoji: "💰",
